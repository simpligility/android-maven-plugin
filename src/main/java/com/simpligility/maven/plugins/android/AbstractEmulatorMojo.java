...
/**
 * AbstractEmulatorMojo contains all code related to the interaction with the
 * Android emulator. At this stage that is starting and stopping the emulator.
 *
...
public abstract class AbstractEmulatorMojo extends AbstractAndroidMojo {
	...
	/**
	 * Even if the device finished booting, there are usually still some things
	 ...
	 */
	private static final String[] BOOT_INDICATOR_PROP_NAMES = { "dev.bootcomplete", "sys.boot_completed",
			"init.svc.bootanim" };
	...
	/**
	 * Determines, which of the properties listed in
	 * {@link #BOOT_INDICATOR_PROP_NAMES} are required to reach the target value
	 * in {@link #BOOT_INDICATOR_PROP_TARGET_VALUES} in order to stop polling.
	 * Since one cannot be picky about what is used as a 'booted' indicator, any
	 * 'signalled' property will be used as an indicator in case of a timeout.
	 */
	 ...
	 * </pre>
	 * 
	 * or configure as properties on the command line as android.emulator.avd,
	 * android.emulator.wait, android.emulator.options and
	 * android.emulator.executable or in pom or settings file as emulator.avd,
	...
	/**
	 * Name of the Android Virtual Device (emulatorAvd) that will be started by
	 * the emulator. Default value is "Default"
	 *
	 * @see com.simpligility.maven.plugins.android.configuration.Emulator#avd
	 */
	 ...
	 *
	 * @see com.simpligility.maven.plugins.android.configuration.Emulator#wait
	 */
	@Parameter(property = "android.emulator.wait")
	private String emulatorWait;

	/**
	 * Additional command line options for the emulator start up. This option
	 * can be used to pass any additional options desired to the invocation of
	 * the emulator. Use emulator -help for more details. An example would be
	 * "-no-skin".
	 *
	 * @see com.simpligility.maven.plugins.android.configuration.Emulator#options
	 */
	 ...
	 *
	 * @see com.simpligility.maven.plugins.android.configuration.Emulator#executable
	 */
	...
	void unlockEmulator(AndroidDebugBridge androidDebugBridge) {
		if (emulatorUnlock) {
			...
			if (devicePort == -1) {
				getLog().info(
						"Unable to retrieve port to unlock emulator " + DeviceHelper.getDescriptiveName(myEmulator));
			} else {
				getLog().info("Unlocking emulator " + DeviceHelper.getDescriptiveName(myEmulator));

				sendEmulatorCommand(devicePort, "event send EV_KEY:KEY_SOFT1:1");
				sendEmulatorCommand(devicePort, "event send EV_KEY:KEY_SOFT1:0");
				sendEmulatorCommand(devicePort, "event send EV_KEY:KEY_SOFT1:1");
				sendEmulatorCommand(devicePort, "event send EV_KEY:KEY_SOFT1:0");
			}
		}
	...
	// TODO Separate timeout params?: New param 'android.emulator.bootTimeout',
	// rename param 'android.emulator.wait' to 'android.emulator.connectTimeout'
	// TODO Higher default timeout(s)?: Perhaps at least for emulators, since
	// they are probably booted or even created on demand
	boolean waitUntilDeviceIsBootedOrTimeout(AndroidDebugBridge androidDebugBridge) throws MojoExecutionException {
		...
		// If necessary, wait until the device is online or the specified
		// timeout is reached
		...
		if (devOnline) {
			...
			// If necessary, wait until the device's system is booted or the
			...
			do {
				try {
					...
					for (int indicatorProp = 0; indicatorProp < BOOT_INDICATOR_PROP_NAMES.length; ++indicatorProp) {
						...
						boolean targetStateReached = (bootIndicatorPropValues[indicatorProp] != null
								&& bootIndicatorPropValues[indicatorProp]
										.equals(BOOT_INDICATOR_PROP_TARGET_VALUES[indicatorProp]));
						if (!targetStateReached) {
							// (re)query
							bootIndicatorPropValues[indicatorProp] = myEmulator
									.getPropertySync(BOOT_INDICATOR_PROP_NAMES[indicatorProp]);
							targetStateReached = (bootIndicatorPropValues[indicatorProp] != null
									&& bootIndicatorPropValues[indicatorProp]
											.equals(BOOT_INDICATOR_PROP_TARGET_VALUES[indicatorProp]));
						}
						anyTargetStateReached |= targetStateReached;
						requiredTargetStatesReached &= BOOT_INDICATOR_PROP_WAIT_FOR[indicatorProp] ? targetStateReached
								: true;

						getLog().debug(BOOT_INDICATOR_PROP_NAMES[indicatorProp] + " : "
								+ bootIndicatorPropValues[indicatorProp] + (targetStateReached ? " == " : " != ")
								+ BOOT_INDICATOR_PROP_TARGET_VALUES[indicatorProp] + " ["
								+ (targetStateReached ? "OK" : "PENDING") + ']');
					}
				} catch (TimeoutException e) {
					// TODO Abort here? Not too problematic since timeouts are
					// used
				...
				if (remainingTime > 0) {
					// consider the boot process to be finished, if all required
	...
	/**
	 * Checks whether the given device has the same AVD name as the device which
	 * the current command is related to. <code>true</code> returned if the
	 * device AVD names are identical (independent of case) and
	 * <code>false</code> if the device AVD names are different.
	 *
	 * @param device
	 *            The device to check
	...
	/**
	 * This method extracts a port number from the serial number of a device. It
	 * assumes that the device name is of format [xxxx-nnnn] where nnnn is the
	 * port number.
	 *
	 * @param device
	...
	/**
	 * Get executable value for emulator from command line options or default to
	 * "emulator".
	 *
	 * @return
	 */
